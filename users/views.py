...
class UserAPIView(viewsets.ViewSet):
    ...
    @swagger_auto_schema(
        ...
        operation_description="Retrieve a list of users with optional filters and search: email, phone_number, role, is_authorized, is_verified, country_code, search_key. Supports pagination with `page` and `page_size`.",
        manual_parameters=[
            ...
            openapi.Parameter('search_key', openapi.IN_QUERY, description="Search by email, first name, last name or role name", type=openapi.TYPE_STRING),
    ...
    def list(self, request):
        ...
        if country_code:
            users = users.filter(country_code__icontains=country_code)
        ...
        if search_key:
            users = users.filter(
                ...
                Q(role__name__icontains=search_key)
            )
    ...
    def create(self, request):
        ...
        webhook_url = "https://chat.googleapis.com/v1/spaces/AAQAvBgiqHU/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=e7DTfvla1JDd8eFaIUR0mc-wBoqzqFW0jf-GghnvuXo"
        # this is for the webhook integration with google chat
        chat_message = {
            ...
            🛡️ Role: {role_name}
            📅 Started On: {user.started_on or "N/A"}"""
        }
        # Send message to Google Chat
        print('successfully sending message to google chat')
        try:
    ...
    def update(self, request, pk=None):
        ...
        return Response({"message": "User updated successfully", "status": status.HTTP_200_OK}, status=status.HTTP_200_OK)
...
class SystemInfoViewSet(viewsets.ViewSet):
    def list(self, request):
        ...
        print("System Info:", system, release, version)

        # Get network interfaces
        interfaces = psutil.net_if_addrs()
        interface_data = {}
        for name, addrs in interfaces.items():
            interface_data[name] = [str(addr.address) for addr in addrs if addr.family == socket.AF_INET]
...
class SerialNumberValidationViewSet(viewsets.ViewSet):
    """
    Validates if the provided Serial Number matches the user's stored Serial Number.
    """
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            ...
            openapi.Parameter('serial_number', openapi.IN_QUERY, description="Serial number to validate", type=openapi.TYPE_STRING, required=True),
        ],
        responses={
            200: openapi.Response(description="Serial number is valid."),
            400: openapi.Response(description="Serial number is invalid."),
...
class MacAddressUpdateViewSet(viewsets.ViewSet):

    @swagger_auto_schema(
        operation_summary="Update User MAC Address",
        operation_description="Takes user ID and new MAC address, and updates the user if found.",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            required=["id", "serial_number"],
            properties={
                "id": openapi.Schema(type=openapi.TYPE_INTEGER, description="User ID"),
                "serial_number": openapi.Schema(type=openapi.TYPE_STRING, description="Serial Number to update"),
            }
        ),
        responses={
            200: openapi.Response(description="MAC address updated successfully."),
            400: openapi.Response(description="Invalid input or user not found."),
        }
    )
    def create(self, request):
        ...
        if not user_id or not serial_number:
            return Response(
                {"message": "Both 'id' and 'serial_number' are required."},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"message": "MAC address updated successfully.","status": status.HTTP_200_OK},
            status=status.HTTP_200_OK
        )
